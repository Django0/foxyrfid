...
#define SIZE_OF_DATA  (4)
      ...
      uint8_t numPunched = data[1] - 1;

      // Puncher ID and number of Foxes gone to
      Serial.print("Puncher ID: ");
      Serial.print(puncherID); Serial.print(", ");
      Serial.print("Number of Foxes: ");
      Serial.println(numPunched);

      for (uint8_t iPage = 1; iPage <= numPunched; iPage++)
      {
        ...
        if (success)
        {
          ...
          if (data[0] == 0x7D) {
            // Repeated punch
            Serial.println("Repeated Fox punched");
          } else {
            Serial.print("Fox: ");
