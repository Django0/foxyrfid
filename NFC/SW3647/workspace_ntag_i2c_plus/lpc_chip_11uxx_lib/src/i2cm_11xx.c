...
{
	uint32_t clockDiv = (SystemCoreClock / busSpeed);
...
uint32_t Chip_I2CM_Read(LPC_I2C_T *pI2C, uint8_t *buff, uint32_t len)
{
	...
	while ((rxLen != len) && (err == 0)) {
		/* wait for status change interrupt */
		if (rxLen == len-1) LPC_I2C->CONCLR = I2C_CON_AA;
		...
		case 0x10:		/* Repeated start condition */
			LPC_I2C->CONCLR = I2C_CON_STA;
			Chip_I2CM_WriteByte(pI2C, buff[rxLen++]); // Send device address
			Chip_I2CM_ClearSI(pI2C);
			break;
		case 0x40:		/* SLA+R sent and ACK received */
			Chip_I2CM_ClearSI(pI2C);
		...
		case 0x50:		/* Data Received and ACK sent */
			...
			Chip_I2CM_ClearSI(pI2C);
			break;
		case 0x58:		/* Data Received and ACK not sent */
			buff[rxLen++] = Chip_I2CM_ReadByte(pI2C);
			break;
