...
//---------------------------------------------------------------------
/* Initialize Timer32_1 to be used for PWM red LED  */
void InitTimer1(void) {
	/* Enable timer 1 clock */
	Chip_TIMER_Init(LPC_TIMER32_1);
	Chip_TIMER_Reset(LPC_TIMER32_1);
	Chip_TIMER_PrescaleSet(LPC_TIMER32_1, 150); //2300 000/15300
	Chip_TIMER_SetMatch(LPC_TIMER32_1,0, 0); //set match register with duty cycle
	Chip_TIMER_SetMatch(LPC_TIMER32_1,1, MAX_DUTY); //set PWM period
	Chip_TIMER_MatchEnableInt(LPC_TIMER32_1, 0); //enable interrupt on match 0
	Chip_TIMER_MatchEnableInt(LPC_TIMER32_1, 1); //enable interrupt on match 1

	//Chip_TIMER_ResetOnMatchEnable(LPC_TIMER32_1, 1); //enable interrupt on match 1
	//Chip_TIMER_Enable(LPC_TIMER32_1);
}

//---------------------------------------------------------------------
/* Enable Timer32_1 to be used for PWM red LED  */
void EnableTimer1(void){
	Chip_TIMER_Enable(LPC_TIMER32_1);
}

//---------------------------------------------------------------------
/* Initialize Timer16_0 to be used for PWM blue and green LED dimming -> NFC cube demo*/
void InitPWM(void) {
	uint32_t timerFreq;
	...
	Chip_TIMER_PrescaleSet(LPC_TIMER16_0, 150);
	Chip_TIMER_SetMatch(LPC_TIMER16_0, 0, 0);
	Chip_TIMER_SetMatch(LPC_TIMER16_0, 1, 0);
	Chip_TIMER_SetMatch(LPC_TIMER16_0, 2, MAX_DUTY);
	LPC_TIMER16_0->PWMC |= 0x03; //enable PWM for chanell 0 and 1
	Chip_TIMER_ResetOnMatchEnable(LPC_TIMER16_0, 2);
	Chip_TIMER_Enable(LPC_TIMER16_0);
	//LPC_IOCON->PIO0[8] = IOCON_MODE_INACT | IOCON_FUNC2;
	//LPC_IOCON->PIO0[9] = IOCON_MODE_INACT | IOCON_FUNC2;
}

//---------------------------------------------------------------------
//setting duty cycle for PWM
//0x00	green
//0x01	blue
//0x02	red
void HW_dimLEDs(uint8_t led, uint8_t duty){

	if(led == 0x02) //RED led
		Chip_TIMER_SetMatch(LPC_TIMER32_1,0, duty);
	else
	{
		if (duty >= MIN_DUTY && duty <= MAX_DUTY)                // check limits
		{
			Chip_TIMER_SetMatch(LPC_TIMER16_0,led,duty);
		}
	}
}

//---------------------------------------------------------------------
//Setting alternate function for PWM LEDs
void InitPWMPins (bool pwm){

	if(pwm)
	{
		//enable PWM on PINO08 and PINO09
		LPC_IOCON->PIO0[8] |= IOCON_FUNC2;
		LPC_IOCON->PIO0[9] |= IOCON_FUNC2;
	}
	else
	{
		//enable default GPIO function
