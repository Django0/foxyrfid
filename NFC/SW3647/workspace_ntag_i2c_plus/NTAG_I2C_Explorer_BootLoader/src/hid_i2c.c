...
static const char *g_fwVersion = "FW v2.4 (" __DATE__ ")";
...
#if (defined(BOARD_NXP_XPRESSO_11U14) || defined(BOARD_NGX_BLUEBOARD_11U24) || defined(BOARD_NXP_XPRESSO_11U37H) || defined(BOARD_NTAG_I2C_EXPLORER))
	...
	while (pIn->resp == HID_I2C_RES_INVALID_CMD) {
		...
		if (HID_I2C_StatusCheckLoop(pHidI2c) == 0) {
			...
			if (handled == 0) {
				...
				case 0x10:	/* Repeated start condition */
					if ((pRWParam->options & HID_I2C_TRANSFER_OPTIONS_NO_ADDRESS) == 0) {
						Chip_I2CM_WriteByte(pHidI2c->pI2C, (pRWParam->slaveAddr) | read);
					}
					break;
